...
measure_native_rust() {
    ...
    for i in $(seq 1 $trials); do
        ...
        /usr/bin/time -v docker run --platform=linux/$arch --rm sangeetakakati/sort-rust-native:latest 2>&1 | tee output_native_rust.log
...
measure_native_tinygo() {
    ...
    for i in $(seq 1 $trials); do
        ...
        # Measure time and memory usage with /usr/bin/time
        /usr/bin/time -v docker run --platform=linux/$arch --rm sangeetakakati/sort-tinygo-native:latest 2>&1 | tee output_native_tinygo.log
...
measure_wasm_rust() {
    ...
    for arch in "${architectures[@]}"; do
        ...
        for i in $(seq 1 $trials); do
            # Cold start time from pull
            docker rmi sangeetakakati/sort-rust-wasm:$arch > /dev/null 2>&1
            script_start_time=$(date +%s%3N)
            echo "Script start time: ${script_start_time} ms"

            # Measure time and memory usage with /usr/bin/time
            /usr/bin/time -v docker run --runtime=io.containerd.wasmtime.v2 --platform=wasm --rm sangeetakakati/sort-rust-wasm:$arch 2>&1 | tee output_wasm_rust.log
...
measure_wasm_tinygo() {
    ...
    for arch in "${architectures[@]}"; do
        ...
        for i in $(seq 1 $trials); do
            # Cold start time from pull
            docker rmi sangeetakakati/sort-tinygo-wasm:$arch > /dev/null 2>&1
            script_start_time=$(date +%s%3N)
            echo "Script start time: ${script_start_time} ms"

            # Measure time and memory usage with /usr/bin/time
            /usr/bin/time -v docker run --runtime=io.containerd.wasmtime.v2 --platform=wasm --rm sangeetakakati/sort-tinygo-wasm:$arch 2>&1 | tee output_wasm_tinygo.log
