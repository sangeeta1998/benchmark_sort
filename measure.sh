...
measure_native_rust() {
    ...
    for i in $(seq 1 $trials); do
        ...
        docker rmi -f  sangeetakakati/sort-rust-native:latest > /dev/null 2>&1
    ...
    for i in $(seq 1 $trials); do
        ...
        docker pull sangeetakakati/sort-rust-native:latest > /dev/null 2>&1
...
measure_native_tinygo() {
    ...
    for i in $(seq 1 $trials); do
        # Cold start time from pull
        docker rmi -f  sangeetakakati/sort-tinygo-native:latest > /dev/null 2>&1
        script_start_time=$(date +%s%3N)
        echo "Script start time: ${script_start_time} ms"
    ...
    for i in $(seq 1 $trials); do
        # Ensuring the image is pulled
        docker pull sangeetakakati/sort-tinygo-native:latest > /dev/null 2>&1
...
measure_wasm_rust() {
    ...
    for arch in "${architectures[@]}"; do
        ...
        for i in $(seq 1 $trials); do
            # Cold start time from pull
            docker rmi -f  sangeetakakati/sort-rust-wasm:wasm > /dev/null 2>&1
            script_start_time=$(date +%s%3N)
            echo "Script start time: ${script_start_time} ms"
            main_start_time=$(docker run --runtime=io.containerd.wasmtime.v1 --platform=wasm --rm sangeetakakati/sort-rust-wasm:wasm 2>&1 | grep "Main function started at:" | awk '{print $5}')
            pull_time=$((main_start_time - script_start_time))
            total_pull_time=$((total_pull_time + pull_time))
        ...
        for i in $(seq 1 $trials); do
            # Ensuring the image is pulled
            docker pull sangeetakakati/sort-rust-wasm:wasm > /dev/null 2>&1
...
measure_wasm_tinygo() {
    ...
    for arch in "${architectures[@]}"; do
        ...
        for i in $(seq 1 $trials); do
            # Cold start time from pull
            docker rmi -f  sangeetakakati/sort-tinygo-wasm:wasm > /dev/null 2>&1
            script_start_time=$(date +%s%3N)
            echo "Script start time: ${script_start_time} ms"
            main_start_time=$(docker run --runtime=io.containerd.wasmtime.v1 --platform=wasm --rm sangeetakakati/sort-tinygo-wasm:wasm 2>&1 | grep "Main function started at:" | awk '{print $5}')
            pull_time=$((main_start_time - script_start_time))
            total_pull_time=$((total_pull_time + pull_time))
        ...
        for i in $(seq 1 $trials); do
            # Ensuring the image is pulled
            docker pull sangeetakakati/sort-tinygo-wasm:wasm > /dev/null 2>&1
