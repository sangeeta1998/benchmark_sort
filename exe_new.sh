...
measure_execution_time() {
    ...
    if [ -z "$runtime" ]; then
        ...
        docker run --name $container_name --rm --mount type=bind,source="$(pwd)",target=/app $image &
    else
        # For Wasm containers with specific runtime
        docker run --runtime=$runtime --platform=$platform --name $container_name --rm --mount type=bind,source="$(pwd)",target=/app $image &
    fi
    container_pid=$!
    wait $container_pid
    end_time=$(date +%s%N)
