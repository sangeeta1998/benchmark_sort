...
measure_execution_time() {
    ...
    echo "Starting container..."
    {
        if [ -z "$runtime" ]; then
            ...
            /usr/bin/time -v docker run --name $container_name --rm --mount type=bind,source="$(pwd)",target=/app $image
        else
            # For Wasm containers with specific runtime
            echo "Running Wasm container with runtime: $runtime"
            /usr/bin/time -v docker run --runtime=$runtime --platform=$platform --name $container_name --rm --mount type=bind,source="$(pwd)",target=/app $image
        fi
